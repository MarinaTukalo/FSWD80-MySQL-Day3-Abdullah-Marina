...
SELECT COUNT(`hire_date`) FROM `employees` WHERE `employees`.`hire_date` > 1985;
SELECT(`first_name`),(`last_name`),(`hire_date`) FROM `employees` WHERE `employees`.`hire_date` >1985; 
...
WHERE employees.hire_date > "1992-01-01" AND departments.dept_no="d008"
